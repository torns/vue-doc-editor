import { EditorContent, BubbleMenu, Editor } from "@tiptap/vue-2"
 ...
 * @param {{(fn:any)=>fn(Vue:any)}} effect 副作用函数，改变外部组件数据
 ...
 */
export const useGenerateMultiple = R.curry((effect, room, wsUrl, currentUser, editable, beforeCreate = n => n) => {
  ...
  effect(that => {
    ...
    // editor.callbacks.update.push(() => {
    //   that.$emit("update:content", editor.getJSON())
 ...
 * @param {((n:any)=>any)|?} beforeCreate 创建前钩子函数
 */
export const useGenerateStatic = R.curry((effect, content = "", editable, beforeCreate = n => n) => {
  const useBeforeHooksAndBack = R.tap(beforeCreate)
  effect(that => {
    const editor = new Editor(
      useBeforeHooksAndBack({
 ...
 * @returns {boolean}
 */
export const keysEq = keys => R.compose(R.equals(0), R.length, R.difference(keys), R.keys)
